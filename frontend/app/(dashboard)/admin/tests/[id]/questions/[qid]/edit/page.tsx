...
import type { Question, SingleResponse } from "@/lib/types";
...
export default function AdminQuestionEditPage({
  ...
  useEffect(() => {
    (async () => {
      try {
        const { data } = await api.get<SingleResponse<Question>>(`/questions/${questionId}`);
        const q = data.data;
        const correctIndex = q.answers?.findIndex(a => a.is_correct) ?? 0;
    ...
    <div className="mx-auto max-w-2xl px-4 py-10">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-semibold">
          {t("tests.admin.question_edit_title")}
        </h1>
        <Link
